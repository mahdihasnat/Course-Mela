...
public interface ViewLogRepository extends JpaRepository<ViewLogEntity, Long> {
    ...
    // select count(distinct videoid) from viewLog entity where studentid = ? and
    // visitTime >= ?
    ...
    Double getTotalWatchTimeOfStudent(Long studentId, ZonedDateTime time);

    // select count(distinct videoid) from viewLog entity where studentid = ? and
    // visitTime >= ? and videoid in (select videoid from video entity where
    // courseid = ?)
    ...
    // select sum(watch_time) from viewLog entity where studentid = ? and visitTime
    // >= ? and videoid in (select videoid from video entity where courseid = ?)
    ...
    Double getTotalWatchTimeOfStudentOfCourse(Long studentId, ZonedDateTime time, Long courseId);

    @Query(value = "select sum(vle.watchTime) " +
            "FROM ViewLogEntity vle " +
            "inner join VideoEntity ve on vle.videoEntity.id = ve.id " +
            "inner join CourseEntity ce on ve.courseEntity.id = ce.id " +
            "where ce.id = ?1 " +
            "and vle.visitTime >= ?2")
    Double getTotalWatchTimeOfCourse(Long courseId, ZonedDateTime startTime);

    // select avg( min(1.0, (select sum(watch_time) from ViewLogEntity where
    // student_entity_id = ? and video_entity_id = id ) /duration ) ) from
    // VideoEntity where course_entity_id = ?
    @Query(value = "SELECT avg( (select sum(watchTime)" +
            " from ViewLogEntity " +
            "where student_entity_id = ?1 " +
            "and video_entity_id = _video.id ) / _video.duration  )  " +
            "from VideoEntity _video " +
            "where course_entity_id = ?2")
    Double getProgressOfCourse(Long studentId, Long courseId);

    @Query(value = "SELECT avg( (select sum(vlE.watchTime)" +
            " from ViewLogEntity vlE " +
            "where student_entity_id = ?1 " +
            "and video_entity_id = _video.id " +
            "and ?3 <= vlE.visitTime " +
            "and vlE.visitTime <= ?4 ) / _video.duration )  " +
            "from VideoEntity _video " +
            "where course_entity_id = ?2")
    Double getProgressOfCourseBetween(Long studentId,
                                      Long courseId,
    ...
    @Query(value = "select sum(vle.watchTime) " +
            ...
            "and ?2 <= vle.visitTime " +
            "and vle.visitTime <= ?3")
    Double getTotalWatchTimeOfVideoBetween(Long videoId,
                                           ZonedDateTime startTime,
                                           ZonedDateTime endTime);
