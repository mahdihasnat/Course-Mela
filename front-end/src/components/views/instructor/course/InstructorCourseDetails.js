...
import { Card, CardBody, CardImg, CardTitle, Container, CardText } from "reactstrap";
        ...
        () => {
            ...
            CourseService.getCourseInstructorView(courseId)
                .then(res => {
                    ...
                    setIsLoading(false);
                    // now fetch the image 
                    ImageService.loadImage(res.data.cover_photo_path).then(response => {

                        // setCoverPhoto(createImageLinkFromByte(response.data));
                        // const srcurl = 
                        setCoverPhoto(window.URL.createObjectURL(new Blob([response.data])));
            ...
            {/* <Card> */}
                ...
                {isLoading ? loadingMessage :
                    (
                        <Container>
                            <Card>
                                <CardImg alt={`cover photo for id ${courseId}`}
                                    src={coverPhoto}
                                    // height={"70%"}
                                />
                                <CardBody>
